<?php
	...
	$sql = "SELECT temp.uploadid,temp.username,temp.first_name,temp.last_name,temp.linktodp,temp.description,temp.path,temp.timestamp,temp.privacy
          FROM
          (
				SELECT UP.uploadid as uploadid,US.username as username,US.first_name as first_name,US.last_name as last_name,US.linktodp as linktodp,Up.description as description,UP.path as path,UP.timestamp as timestamp,UP.privacy as privacy FROM user US,upload UP WHERE US.username = UP.username AND privacy = 0  
          ) 
		  AS temp 
